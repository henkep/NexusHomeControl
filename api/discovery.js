...
async function discoverHoneywell(username, password) {
    ...
    try {
        ...
        let resp = await honeywellRequest('POST', '/portal/', postData);
        
        // Follow all redirects to reach the zones/dashboard page
        let redirectCount = 0;
        while (resp.location && redirectCount < 5) {
            console.log('Following redirect to:', resp.location);
            resp = await honeywellRequest('GET', resp.location, '');
            redirectCount++;
        }
        
        // Extract unique device IDs from control links in the page
        const deviceLinks = resp.body.match(/\/portal\/Device\/Control\/(\d+)/g) || [];
        const uniqueIds = [...new Set(deviceLinks.map(l => l.split('/').pop()))];
        
        console.log('Found device IDs:', uniqueIds);
        
        if (uniqueIds.length === 0) {
            console.log('No devices found in page');
            return [];
        }
        
        // Fetch each device's control page to get its name
        const devices = [];
        for (const id of uniqueIds) {
            try {
                const ctrl = await honeywellRequest('GET', `/portal/Device/Control/${id}`, '');
                // Try to extract device name from page
                const nameMatch = ctrl.body.match(/<h1[^>]*>([^<]+)<\/h1>/i) || 
                                  ctrl.body.match(/id="DeviceName"[^>]*value="([^"]+)"/i) ||
                                  ctrl.body.match(/zoneName['":\s]+['"]([^'"]+)/i);
                let name = nameMatch ? nameMatch[1].trim() : `Thermostat ${id}`;
                // Clean up name (remove " Control" suffix if present)
                name = name.replace(/\s*Control\s*$/i, '').trim();
                ...
                console.log(`Failed to get details for device ${id}:`, e.message);
                devices.push({
                    id: parseInt(id),
                    name: `Thermostat ${id}`,
                    type: 'thermostat'
